# Push to nudgeeducation/policy-vault

Walkthrough for the first push to GitHub. Run these from your terminal (the Cowork sandbox can't push to GitHub on your behalf — your terminal has the credentials).
...
```bash
cd "$HOME/Documents/Claude/Projects/Nudge Education/nudge-policy-vault"
```

### 2. Pre-flight cleanup

A few files need removing before the initial commit — the Cowork sandbox couldn't delete them because of how your Documents folder mount permissions work, but your terminal can. These are leftover Quartz upstream artifacts that would create unnecessary GitHub Actions noise:

```bash
# Remove the leftover .git from the abandoned clone attempt
rm -rf .quartz-temp

# Remove upstream Quartz workflows we don't need (we have our own deploy.yml)
...
# Verify only our deploy.yml remains
ls .github/workflows/
# Should show: deploy.yml
```

**Optional — replace the README:**

The repo's `README.md` currently contains the Quartz upstream README ("Quartz v4 — One who works with the door open..."). This will be the first thing visitors see on the GitHub repo page. If you'd prefer a Nudge Education-specific README, replace it before committing — for example:
